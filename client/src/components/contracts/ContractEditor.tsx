...
import LexiContextMenu from './contextmenu/LexiContextMenu';
import { useClauseComposer } from '@/hooks/use-clause-composer';
...
export default function ContractEditor({ contractId: propContractId }: ContractEditorProps) {
  // Default fallback user in case the context isn't available
  let user = { id: 1, fullName: 'Demo User' };
  
  // Log the contractId passed as prop for debugging
  console.log("Contract ID from props:", propContractId);
  
  // Try to get the real user from context, but don't crash if unavailable
  try {
    const auth = useAuth();
    if (auth && auth.user) {
      user = auth.user;
  ...
  const { data: contract, isLoading, isError } = useQuery<Contract>({
    queryKey: ['/api/contracts', propContractId],
    // Use the default query function configured in QueryClient
    enabled: !!propContractId && propContractId > 0,
  });
  
  // Set up mutations
  const updateContractMutation = useMutation({
    mutationFn: async (data: Partial<Contract>) => {
      const response = await apiRequest("PATCH", `/api/contracts/${propContractId}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/contracts', propContractId] });
      toast({
        title: "Changes saved",
  ...
  const enhanceClauseMutation = useMutation({
    mutationFn: async (data: { action: string; content: string; clauseId?: string }) => {
      const response = await apiRequest("POST", `/api/contracts/${propContractId}/enhance`, data);
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const generatePdfMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", `/api/contracts/${propContractId}/pdf`, {});
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const saveAsTemplateMutation = useMutation({
    mutationFn: async (data: { name: string; description: string }) => {
      const response = await apiRequest("POST", `/api/contracts/${propContractId}/save-as-template`, data);
      return response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const sendEmailMutation = useMutation({
    mutationFn: async (data: { recipients: string[]; subject: string; message: string }) => {
      const response = await apiRequest("POST", `/api/contracts/${propContractId}/send-email`, data);
      return response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const signContractMutation = useMutation({
    mutationFn: async (data: { signerName: string; signerEmail: string; signatureData: string }) => {
      const response = await apiRequest("POST", `/api/contracts/${propContractId}/sign`, data);
      return response.json();
    },
    onSuccess: () => {
      toast({
  ...
  useEffect(() => {
    if (contract) {
      ...
      // Always update the editor with contract content when available
      if (editorRef.current && contract.content) {
      ...
      console.log("Contract loaded:", {
        ...
        clausesCount: contract.clauses ? contract.clauses.length : 0
      });
    }
  }, [contract]);
  
  // Make sure editor content is loaded even if ref isn't ready in first render
  useEffect(() => {
    // This secondary effect ensures content gets loaded properly
    if (editorRef.current && contractContent && !editorRef.current.innerHTML) {
      console.log("Secondary initialization of editor content");
      editorRef.current.innerHTML = contractContent;
    }
  }, [contractContent, editorRef.current]);
  ...
  // Setup clause composer hook
  const { enhanceClause, composeClause, isComposing, generatedClause, reset: resetClauseComposer } = useClauseComposer({
    contractId: propContractId,
    jurisdiction: contract?.jurisdiction || 'India',
    contractType: contract?.type || 'standard',
    tone: 'balanced',
    onSuccess: (result) => {
      // If we receive a successful result, update the contract or add new clause
      if (result) {
        const updatedClauses = [...clauses];
        
        // Check if this is an existing clause that was updated
        const existingIndex = updatedClauses.findIndex(c => c.id === result.id);
        
        if (existingIndex >= 0) {
          // Update existing clause
          updatedClauses[existingIndex] = result;
        } else {
          // Add new clause
          updatedClauses.push(result);
        }
  ...
  // Handle context menu action selections
  const handleContextMenuAction = (action: string, text: string) => {
    ...
    // Create a temporary clause from the selected text
    ...
    // Use the clause composer to enhance the text based on the selected action
    enhanceClause(action, tempClause, {
      jurisdiction: contract?.jurisdiction || 'India',
      contractType: contract?.type || 'standard',
      tone: 'balanced'
    });
  ...
  // Enhance a clause with AI (using direct mutation)
  const enhanceClauseWithMutation = async (action: string, content: string, clauseId?: string) => {
    setIsEnhancing(true);
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col px-6 py-4">
      {/* Use our new LexiContextMenu component */}
      <LexiContextMenu 
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ 
                    __html: (editorRef.current && editorRef.current.innerHTML) || 
                             ...
                             "<div class='p-4 text-gray-500'>No content available</div>"
                  }}
                  style={{ overflow: 'auto' }}
                />
              </TabsContent>
      ...
      {/* Enhance Clause Dialog */}
      <Dialog open={showClauseDialog} onOpenChange={setShowClauseDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{enhanceAction.charAt(0).toUpperCase() + enhanceAction.slice(1)} Clause</DialogTitle>
            <DialogDescription>
              Lexi AI will help you improve your contract clause.
            </DialogDescription>
          </DialogHeader>
          
          <div className="grid gap-4">
            <div>
              <Label htmlFor="original-clause">Original Text</Label>
              <div id="original-clause" className="mt-1 rounded-md border border-gray-200 p-3 text-sm text-gray-700">
            ...
            {isEnhancing ? (
              <div className="flex items-center justify-center py-8">
                <div className="text-center">
                  <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent"></div>
                  <p className="mt-2 text-gray-500">Lexi is thinking...</p>
                </div>
              </div>
            ) : enhanceResult ? (
              <div>
                <Label htmlFor="enhanced-clause">Enhanced Text</Label>
                <div id="enhanced-clause" className="mt-1 rounded-md border border-gray-200 p-3 text-sm text-gray-700">
                ...
                {enhanceResult.explanation && (
                  <div className="mt-4">
                    ...
                    <div id="explanation" className="mt-1 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
                      {enhanceResult.explanation}
                    </div>
                  </div>
                )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowClauseDialog(false)}>Cancel</Button>
              ...
              <Button 
                ...
                onClick={() => {
                  if (selectedClause) {
                    ...
                    enhanceClauseWithMutation(
                      enhanceAction,
                      selectedClause.content,
                      selectedClause.id !== 'temp' ? selectedClause.id : undefined
                    );
                  }
                }}
              >
                {isEnhancing ? 'Processing...' : 'Process with Lexi AI'}
              </Button>
            )}
            ...
            {enhanceResult && (
              <Button 
                onClick={() => {
                  ...
                  if (enhanceResult.enhancedContent && editorRef.current) {
                    if (selectedClause?.id.startsWith('temp-')) {
                      // Insert as new clause
                      const newClause: Clause = {
                        id: `clause-${Date.now()}`,
                        title: selectedClause.title || "New Clause",
                        content: enhanceResult.enhancedContent,
                        explanation: enhanceResult.explanation || ""
                      };
                      
                      addClause(newClause);
                    } else {
                      // Replace existing clause
                      // Logic to replace existing clause in the document
                      document.execCommand('insertText', false, enhanceResult.enhancedContent);
                    }
